...
https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API/Drag_operationsorg/en-US/docs/Web/API/HTML_Drag_and_Drop_API/Recommended_drag_types
...
- **dragstart** - событие срабатывает на перетаскиваемом элементе при начале перетягивания. Срабатывает _только_ на элементе, т.е. не срабатывает при DnD из ОС или т.п. (другой страницы?)
  <!-- TODO: надо бы попробовать передать в Drag Data что-нибудь из одной страницы в другую -->
- **dragenter** - событие срабатывает на целевом элементе, когда перемещаемый элемент попадает на элемент-назначение
