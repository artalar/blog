...
С первого взгляда может показаться что это очень похожий код, результат которого тоже будет очень похож. Но, согласно спецификации сортировка свойств объекта имеет некоторые особенности, в результате которых в первом случае результат будет `{1: null, 2: null}`, а во втором `{2.0: null, 1.0: null}`. Как можно заметить, порядок свойств поменялся. Эта логика не интуитивно понятна и описывается, как раз, семантикой.
...
Сравните еще раз код `1` и `2`. С функциональной точки зрения разницы в нем нет, но если `array` будет обернут в прокси, то каждый вызов `array[i]` будет "утяжелен" перехватчиком, поэтому у этого, казалось бы, одинакового кода может быть совершенно различная производительность.
...
рендерМетоды увеличивают связанность (пропсы разбросаны по ним) и не дебажатся в девтулзах

![](lost-chapters-of-frontend-book/2019-03-18-10-21-48.png)

Бейте все на компоненты
...
Попробуйте не использовать JSX для контейнеров. Обратите внимание что этот код больше похож на использование хуков
...
Под семантикой в языке подразумевается **смысловое значение**, т.е. наличие семантики - это наличие явности выражения мысли. Или, говоря проще, семантика - это контекст.
