...
- Структура проекта
  ...
  - feature-based separation VS technology-based separation
  - фрактальная VS глобальная
  > Хороший пример структуры - [Атомарный дизайн (Atomic Design)](https://medium.com/атомарный-дизайн/atomic-design-foreword-4960ad17bc1a)
...
- Порядок смысловых конструкций кода `bigArray.filter(some => some === this.props.v)` VS `const { v } = this.props; bigArray.filter(some => some === v)`.
  > Показательным примером важности семантики расстановки блоков кода являются новые хуки в React.js<!-- TODO: вставить картинку -->.
- Proxy, get, set
...
- Берите готовые сборки`this.props`
  - CRA
  - Next.js
  > redux больше фреймворк, чем некст
- **No** Hype Driven Development
  > Берите только те библиотеки в которых чувствуете нужду (обоснованную).

  > Написать сервисный проект за 1-2 месяца на новом стеке будет полезнее и дешевле, чем знакомится с ним в продакшене.
