...
#### Drag Event (событие перетаскивания)
...
- Drag Feedback Image (визуальная реакция перетаскивания)
...
- Drop Feedback (визуальная реакция цели перетаскивания)
...
- Performing a Drop (обработка отпускания)
  - Что бы закончить перетаскивание пользователю нужно отпустить кнопку мыши (или нажать Esc). Если это быдет сделано над валидной целью перетаскивания (см. Drop Targets) - тогда будет вызван обработчик "ondrop". В событии у свойства `dataTransfer` можно вызвать метод `getData(type)` указав необходимый тип данных. Если событие было обработано, то рекомендуется выполнить `event.preventDefault()` для обработки события браузеров по умолчанию: например для ссылки - это ее открытие
- Finishing a Drag (окончание перетаскивания)
  - По завершению перетаскивания на перетаскиваемом элементе будет вызван обработчик события dragend<!-- если нода была удалена или перемещена, то в Gecko событие не будет вызвано https://bugzilla.mozilla.org/show_bug.cgi?id=460801 -->. Из свойства `dropEffect` можно определить какой тип перетаскивания был осуществлен (`none` означает что произошла отмена (`Esc`) или <!-- TODO: проверить -->отпускание не над валидной целью).
  - Свойства события `screenX` и `screenY` будут относительно того экрана, где было завершено перетаскивание, даже если это было в другом окне
...
- **drop** - событие срабатывает на целевом элементе при отпускании элемента над целью перетаскивания. Событие не сработает, если пользователь отменит перетаскивание, например, нажав Escape <!-- TODO: что еще кроме ecs может быть? -->
